...
if __name__ == "__main__":
    if len(sys.argv) < 5 or len(sys.argv) > 6:
        ...
        print(
            'Usage: python ac_xai_shap.py modelId numberBackgroundSamples numberExplainedSamples maxDisplay [modelType]')
    else:
        modelId = sys.argv[1]
        ...
        if len(sys.argv) == 6:
            ...
            def read_csv_with_fallback(file_path):
                ...
                # If it fails, try reading with semicolon delimiter
                try:
                    data = pd.read_csv(file_path, sep=";")
                    if 'output' in data.columns:
                        return data
                ...
                raise ValueError(
                    f"Unable to read the file: {file_path} with both ',' and ';' delimiters or 'output' column not found.")
        ...
        train_data = read_csv_with_fallback(train_data_path)
        test_data = read_csv_with_fallback(test_data_path)

        # Ensure 'output' column exists before dropping
        if 'output' in train_data.columns:
            X_train = train_data.drop(columns=['output'])
            y_train_orig = train_data['output']
        else:
            raise KeyError("'output' column not found in train_data")

        if 'output' in test_data.columns:
            X_test = test_data.drop(columns=['output'])
            y_test_orig = test_data['output']
        else:
            raise KeyError("'output' column not found in test_data")

        # if 'output' in train_data.columns:
        #   X_train = train_data.drop(columns=['output'])
        #   y_train_orig = train_data['output']
        # else:
        #   print("Column 'output' not found in DataFrame")
        #
        X_train = train_data.drop(columns=['output'])
        ...
        time_taken = timeit.timeit(
            lambda: running_shap(model, numberBackgroundSamples, numberExplainedSamples, maxDisplay, modelType),
            number=generation_iters)
