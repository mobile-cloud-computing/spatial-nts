...
router.get('/datasets', async (req, res, next) => {
  try {
    ...
    const allDatasets = files.filter(file => path.extname(file) === '.csv');
...
router.post('/retrain', async (req, res, next) => {
  const { retrainACConfig } = req.body;
  if (!retrainACConfig) {
    res.status(401).send({
    ...
    if (retrainStatus.isRunning) {
      ...
      await startRetrainModelAC(retrainACConfig, (retrainStatus) => {
...
router.post('/retrain/:modelId', (req, res) => {
  ...
  startRetrainModelAC({ modelId, ...datasetsConfig }, (retrainStatus) => {
    if (retrainStatus.error) {
